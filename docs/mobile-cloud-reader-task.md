...
- 漫画库导入支持：
  - 压缩包：ZIP / CBZ。
  - 图片：手动多选图片作为一本漫画。
  - 文件夹：通过 Android SAF 原生目录授权选择已解压漫画目录。
- 导入名称可选填：
  - 填写时使用用户输入。
  - 留空时压缩包用文件名。
  - 留空且只选择图片时用第一张图片名。
- 手机上漫画分散在多个目录时，可多次授权不同目录；后续再做“记住多个目录并重扫”。

验收：

- 三杠菜单和底部导航都能进入设置页。
- 能从设置页导入 ZIP/CBZ、图片组和授权文件夹。
- 导入后书库出现新漫画。
- 手动填写导入名称时，书库显示该名称。
...
- Android WebView 的普通文件选择不支持稳定选择文件夹；文件夹导入走 SAF 原生目录授权。
