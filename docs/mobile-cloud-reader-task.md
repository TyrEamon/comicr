...
MVP 建议选第 1 种：手机 App 自包含，先把阅读、下载、云盘导入跑通。PC 端代码作为参考，不直接运行在 APK 里。
...
- 手机端底部导航和顶部栏。
- 本地书库页：显示本机/导入的漫画，支持 ZIP/CBZ、已解压文件夹、图片组导入。
- 漫画详情页：封面、标题、进度、阅读入口。
- 阅读器页：沉浸阅读、进度保存。
- 下载页：粘贴链接下载漫画/本子。
- 云盘页 MVP：浏览 provider 文件，通过本地导入 provider 导入压缩包、图片目录或图片组到本地书库。
...
- 搜索按钮在书库页聚焦标题搜索；在其他页面点击时跳回书库并聚焦搜索。
...
1. 打开 APK 进入书库。
2. 从书库页或云盘页的本地导入入口导入一个图片目录、图片组或压缩包。
3. 书库出现新漫画。
4. 进入详情页。
5. 点击 `开始阅读` 阅读。
6. 退出后再次进入能恢复进度。
...
- APK 里能跑通：云盘/本地导入 -> 书库 -> 详情 -> 阅读 -> 进度保存 -> 链接下载。
