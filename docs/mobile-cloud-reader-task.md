...
- 漫画库导入支持：
  - 压缩包：ZIP / CBZ。
  - 图片：手动多选图片作为一本漫画。
  - 文件夹：需要 Android SAF 原生目录授权，不能用普通 HTML `webkitdirectory` 实现。
- 导入名称可选填：
  - 填写时使用用户输入。
  - 留空时压缩包用文件名。
  - 留空且只选择图片时用第一张图片名。
- 手机上漫画分散在多个目录时，当前先用图片多选或 ZIP/CBZ 兜底；后续 Android SAF 版本再做“授权并记住多个目录”。

验收：

- 三杠菜单和底部导航都能进入设置页。
- 能从设置页导入 ZIP/CBZ 和图片组。
- 文件夹按钮不高亮伪装为已支持；点击时说明需要 Android SAF。
- 导入后书库出现新漫画。
- 手动填写导入名称时，书库显示该名称。
...
1. Android 目录体验：MVP 使用系统文件选择器导入压缩包或图片组；文件夹导入需要 SAF 原生插件来记住多个分散目录。
...
5. 漫画文件格式：第一版已支持 ZIP/CBZ、图片组；后续是否增加 SAF 文件夹、RAR/7z。
...
- Android WebView 的普通文件选择不支持稳定选择文件夹；文件夹导入必须走 SAF 原生目录授权。
