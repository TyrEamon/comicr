...
export const useLibraryStore = defineStore('library', () => {
  ...
  async function importTextFile(file: File, title?: string, refreshAfter = true) {
    const manga = await libraryService.importTextFile(file, title)
    if (refreshAfter) await refresh()
    return manga
  }
  ...
  async function importImageRefs(
    ...
    return libraryService.importImageRefs(title, images, source, options)
  }
  ...
  async function importArchiveRefs(
    ...
    const manga = await libraryService.importArchiveRefs(title, images, options)
    if (refreshAfter) await refresh()
    return manga
  }
